...
public class ScheduledTransactionJob {
  ...
  public void executeScheduledTransactions() {
    ...
    List<ScheduledTransaction> scheduledTransactions = scheduledTransactionRepository.findAllByNextRunAtBeforeAndStatus(
        Instant.now(), ScheduledStatus.ACTIVE);
    log.info("scheduledTransactions size: {}", scheduledTransactions.size());
